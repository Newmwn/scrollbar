...
export class DomHandler {
  ...
  public static isEventValid(event) {
    let elem = document.createElement('div');
    let flag = `on${event}` in elem;
    elem = null;
    return flag;
  }
   ...
   * @param target element clicked (panel options)
   * @param spacer gap between select and panel options
   */
  public static relativePosition(element: any, target: any, spacer: number = 6, elemHeight?): void {
    const targetHeight = target.offsetHeight;
    const targetOffset = target.getBoundingClientRect();
    const viewport = this.getViewport();
    let parents = this.getParents(element).map(item => {
      return item.style != null && item.style?.transform != '';
    });
    element.style.position = 'fixed';
    element.style.width = targetOffset.width + 'px';
    element.style.left = (parents.some(item => item == true) ? 0 : targetOffset.x) + 'px';

    let elementDimensions = elemHeight ?? element.offsetHeight;
    let top: number;
    if (targetOffset.top + targetHeight + elementDimensions > viewport.height) {
      top = -1 * elementDimensions - 2;
      ...
      element.style.top = `calc(${targetOffset.y - elementDimensions - spacer}px )`;
    } else {
      top = targetHeight;
      element.classList.add('top');
      target.classList.add('top');
      let offSetTop = targetOffset.height + (parents.some(item => item == true) ? target.offsetTop : targetOffset.y);
      element.style.top = `calc(${offSetTop}px + ${TBU.tbu(TBU.px(spacer))})`;
    }

    // change select option position to fixed, to overflow all containers
    element.style.position = 'fixed';
    element.style.width = targetOffset.width + 'px';
    element.style.left = (parents.some(item => item == true) ? 0 : targetOffset.x) + 'px';
  }
  ...
  static getScrollableParents(element: any) {
    ...
    if (element) {
      ...
      const overflowCheck = (node: any) => {
        ...
        let styleDeclaration = getComputedStyle(node, null);
        return (
      ...
      for (let parent of parents) {
        if (parent.nodeName === 'NC-SCROLLBAR') {
          scrollableParents.push(parent.firstChild);
          continue;
        }
