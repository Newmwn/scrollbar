import { ShadowDomHelper } from '../../../utils/helpers/shadow-dom-helper';
...
export class NpVirtualKeyNav {
  ...
  constructor(config: KeyNavConfig) {
    ...
    if (!this.config.ignoreInitialEventListeners) this.initEventListeners(true);
  }
  ...
  updateListItems() {
    ...
    if (!this.listItems?.length && this.listItems[0] != this.firstItemFocus) {
    ...
    if (this.listItems?.length > 0 && !this.firstItemFocus) {
  ...
  initEventListeners(focusFirstItem: boolean = false) {
    if (this.config.focusFirstItem && focusFirstItem && this.listItems.length) {
      this.focusElementByPosition(0);
    }
   ...
   * @param dataIndex - The data index of the view port item
   */
  focusElementByIndex(dataIndex) {
    requestAnimationFrame(() => {
      const elem = this.config.scrollerRef.shadowRoot.querySelector(`.viewport-item[data-index="${dataIndex}"]`);

      if (elem == undefined) {
        console.warn(`the element with the data index of ${dataIndex}, does't exist`);
        return;
      }
  ...
  /** This method is used to give focus to an list element
   * @param position - The position of the view port item in the item container
   */
  focusElementByPosition(position) {
    requestAnimationFrame(() => {
      const elem = this.config.scrollerRef.shadowRoot.querySelectorAll(`.viewport-item`)[position];

      if (elem == undefined) {
        console.warn(`the element in the position ${position}, does't exist`);
        return;
      }
  ...
  private focusElement(elem) {
    const elementToFocus = ShadowDomHelper.searchElement(elem, `.${this.config.elemToFocus}`);

    if (elementToFocus == null) {
      console.warn(`the element to focus wasn't found`);
      return;
    }

    this.elemToFocus = elementToFocus;
  ...
  private scrollPreviousElement($event) {
    ...
    const itemIndex: number = Number(itemTarget.getAttribute('data-index')) + this.config.extraIndex;
    ...
    if (itemPosition < scroll) {
      (this.config.scrollerRef as any).scrollToPreviousItem();
  ...
  private scrollNextElement($event) {
    ...
    const itemIndex: number = Number(itemTarget.getAttribute('data-index')) + 1 + this.config.extraIndex;
    ...
    if (itemPosition > scroll + scrollerSize) {
      (this.config.scrollerRef as any).scrollToNextItem();
      this.elemToFocus.focus({ preventScroll: true });
      this.toUpdateFocus = true;
    } else {
      this.elemToFocus.focus({ preventScroll: true });
